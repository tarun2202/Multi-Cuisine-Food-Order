...
public class VendorServiceImpl implements VendorService {
	...
	public DeliveryDTO delivery() {
		...
		for (Orders order : oList) {
			...
			Address address = addressDao.getByCustomerId(customer.getId()).get(0);
			String delAddress = address.getStreet()+", "+address.getCity()+", "
			...
			delDto.setAddress(delAddress);
			delDto.setDishName(nameList);
		}
