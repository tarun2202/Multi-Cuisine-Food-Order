...
Vendors vendor = vendorDao.findById(vendorId).orElse(null);
	...
	public AdminDashboardDTO adminDashboard() {
		...
		adminDto.setCustomerCount(customerDao.findAll().size());
		adminDto.setDishCount(dishDao.findAll().size());
		adminDto.setOrderCount(orderDao.findAll().size());
		adminDto.setVendorCount(vendorDao.findAll().size());
		double revenue = 0.0;
