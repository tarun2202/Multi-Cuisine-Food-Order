...
function EditDish() {
    ...
    axios.put(`http://localhost:8080/dishes/${dishId}`,
    {
         "dishName" : dname,
      ...
      .then(res=>{
        debugger
          if(res.data.message==="Dish Updated!"){
    ...
    const getDish=()=>{
        ...
        axios.get(`http://localhost:8080/dishes/get/${dishId}`,{
            headers:{
              'Authorization': 'Bearer '+ token
            }
          }).then(res=>{
            setDname(res.data.dishName);
            setDimage(res.data.dishImage);
            setCategory(res.data.category);
            setType(res.data.cuisine);
            setDdescription(res.data.description);
  ...
  return (
    ...
    <div className="user-reg-form">
               <h2 style={{fontWeight:"bolder",color:"white"}}>Edit A Dish</h2>
              ...
              <div className="button-container">
               <button className="btn btn-dark" onClick={updateDish} >Update</button>
               </div>
               </div>
             </div>
             <Footer/>
